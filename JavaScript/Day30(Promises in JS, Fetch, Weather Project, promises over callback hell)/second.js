...
//Promise based implementation (no callback hell)
function placeOrder(cart){
    console.log("Talking with Domino's");
     
    const pr = new Promise(function(resolve,reject){ //executor function
        ...
        setTimeout(()=>{
           ...
           const food_available = true;
            ...
            console.log("Order Placed Succesfully");
            const order = {orderId: 221 , food: cart , restaurant: "Dominos", location:"Dwarka"}
            resolve(order); //resolve value goes to then block
            }
            else{
                reject("Items Out of Stocks"); //reject value goes to catch block
            }
           },2000)
    ...
    return pr; //goes to placeOrder(cart).then()
}
