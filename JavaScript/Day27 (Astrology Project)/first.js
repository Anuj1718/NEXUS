...
form.addEventListener('submit',(event)=>{
    ...
    const Name = document.getElementById('name').value; //can do using FormData object too
    const SurName = document.getElementById('surname').value;
    const Day = Number(document.getElementById('day').value); // as everything this way (input from form) comes in string
    ...
    let index = Math.floor(Math.random()*20); //0-19
    const fourth_message =  victimCardCompliments[index]; //made it random


    index = (Name.length * SurName.length * Year)%30; //as we didnt wanna make it random, refresh karke fir kare to same recommendation aye
