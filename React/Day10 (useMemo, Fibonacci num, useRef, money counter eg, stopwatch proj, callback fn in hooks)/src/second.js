...
function StopWatch(){
   ...
   const [isRunning , setIsRunning] = useState(false); //as initially no interval is running
   ...
   function stop(){
      if(isRunning){
      clearInterval(intervalRef.current); // did this to clear the interval, i.e to stop the stopwatch
      intervalRef.current=null; //did this to avoid memory leak, not necessary
      setIsRunning(false);
      }
   }
